	...
	if (!loc.equals(original)) {
		...
		if (req.aborted()) {
			...
			if (res) {
				...
				self.history().pushState(self.state(),null,String(original));
			} else if (!params.push) {
				self.history().replaceState(self.state(),null,String(original));
			};
		...
		if (!req.aborted()) {
			...
			if (params.push) {
				self.history().pushState(params.state || self.state(),null,String(self._location));
			} else {
				self.history().replaceState(params.state || self.state(),null,String(self._location));
				self;
			};
...
Router.prototype.go = function (url,state){
	...
	this.refresh({push: true,location: loc,state: state});
	return this;
};
...
Router.prototype.replace = function (url,state){
	if(state === undefined) state = {};
	let loc = this._location.clone().update(url,state);
	return this.refresh({replace: true,location: loc,state: state});
